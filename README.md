<h1>プロジェクト</h1>
<p>色んなキャラクターを画面に表示して
<p>うまく投票ができるホームページを作るのがこのプロジェクトの目標でした。

<h1>環境</h1>

<p>Ruby on Rails(Rails 5.0.7.1)
<p>ruby 2.3.6p384
<p>Docker

<h1>技能</h1>
<p>キャラクターの名前と写真を登録したら名前と仮面が投票ボタンと一緒に画面に表示します。（今は写真の登録は出来ません）
  <p>登録したキャラクターの修正と削除も出来ます。
<p>投票したらResult（結果）画面でカウントが上がります。
<p>Character_infomation（キャラックター情報）の画面で詳しいキャラクターの情報を見ています。

<h1>screen shot/流れ</h1>
<h2>トップ画面</h2>
<img width="1280" alt="2019-02-27 12 11 20" src="https://user-images.githubusercontent.com/44548905/53464079-61135480-3a8c-11e9-8560-bbaed15b7aff.png">
  
  
  <h2>キャラクター登録方法</h2>
  <img width="466" alt="2019-02-27 12 47 55" src="https://user-images.githubusercontent.com/44548905/53464555-f5ca8200-3a8d-11e9-9c53-24d315a0cdbc.png">
<p>トップ画面の右にあるキャラクター登録をクリック

  <img width="1270" alt="2019-02-27 12 14 50" src="https://user-images.githubusercontent.com/44548905/53464083-61abeb00-3a8c-11e9-9640-e1f5965b020c.png">
  
  <img width="1273" alt="2019-02-27 12 14 59" src="https://user-images.githubusercontent.com/44548905/53464084-61abeb00-3a8c-11e9-9cb0-afa43c0c0a65.png">
  
  
  <p>キャラクターの名前とイメージを書いて登録する。（イメージ登録は機能しません）
  
  <img width="1274" alt="2019-02-27 12 15 10" src="https://user-images.githubusercontent.com/44548905/53464085-61abeb00-3a8c-11e9-8bab-38fe3722cfb5.png">
  登録が終わったらトップ画面に表示します。
  
<h2>投票の方法</h2>

<img width="1269" alt="2019-02-27 12 15 24" src="https://user-images.githubusercontent.com/44548905/53464086-62448180-3a8c-11e9-89d2-a01d8c4333cc.png">
<p>投票をクリックした画面で、確認するポップアップが出て投票をします。
<p>青の確認ボタンをクリックしたら投票完了です。
<img width="1280" alt="2019-02-27 13 01 57" src="https://user-images.githubusercontent.com/44548905/53465110-eb10ec80-3a8f-11e9-8c36-3c18475d62d5.png">
    投票が終わったらResult（結果）画面で確認出来ます。
  
<h2>登録したキャラクターを修正と削除する方法</h2>
  <img width="1275" alt="2019-02-27 12 16 18" src="https://user-images.githubusercontent.com/44548905/53464089-62448180-3a8c-11e9-87f6-ab4bb8e92d93.png">
  <p>トップ画面でキャラクターの下にある'修正'ボタンをクリックした画面です。
  <p>登録と同じ方法で修正が出来ます。
<img width="1280" alt="2019-02-27 13 01 33" src="https://user-images.githubusercontent.com/44548905/53465103-e9dfbf80-3a8f-11e9-806d-2de48ad06b63.png">
    修正しても投票した結果は変わりません。
<img width="1241" alt="2019-02-27 12 16 43" src="https://user-images.githubusercontent.com/44548905/53464091-62dd1800-3a8c-11e9-9d37-9d1c58c18469.png">
   <p>トップ画面でキャラクターの下にある'削除'ボタンをクリックした画面です。
     <p>投票と同じ方法で青の確認ボタンをクリックしたら削除が出来ます。
       <img width="1279" alt="2019-02-27 13 13 23" src="https://user-images.githubusercontent.com/44548905/53465513-90789000-3a91-11e9-89bd-0af22abefe32.png">
       <p>投票した結果も一緒に削除します。
  
  
<img width="1272" alt="2019-02-27 02 22 29" src="https://user-images.githubusercontent.com/44548905/53433130-2f23d300-3a37-11e9-955a-fa81a2a6b991.png">
<p>Character_infomation（キャラックター情報）画面です。
<p>キャラクターにはlinkが張っています。


<img width="1256" alt="2019-02-27 02 04 00" src="https://user-images.githubusercontent.com/44548905/53431569-1fef5600-3a34-11e9-8115-d67364084262.png">
<p>キャラクターをクリックしたら出る画面です
<p>色んな情報が書いてます特に原作の項目にはどこから生まれたキャラクターかを、教えるリンクが張っています。


 <img width="1278" alt="2019-02-27 02 50 15" src="https://user-images.githubusercontent.com/44548905/53434675-7790c000-3a3a-11e9-8008-fe7208cc0923.png">
キャラクター情報を登録する画面です（エラーはないんですが作動はしません）
</div>

<h1>あとがき</h1>
<p>初めてのITプロジェクトでした
<p>ITの経験が全然ない状態だったから辛かったんです。
<p>今もわからない部分が沢山あるけど、どの部分をわからないかも、わからない状態で苦しいんです。
<p>多分、何ヶ月ぐらいは今の状態が繋がるんだと思います。
<p>プロジェクトをおしまいする時は、納得出来ないエラーとかは出なくてよかったでした。
<p>狙った部分を考えて綺麗に作動するのを作るのは面白いでした。
<p>出来ない部分も結構あったけど勉強する時間も苦しいけど楽しい時間でした。
